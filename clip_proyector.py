...
# Code from https://github.com/orpatashnik/StyleCLIP/blob/main/criteria/clip_loss.py
class CLIPLoss(torch.nn.Module):

    def __init__(self, stylegan_resolution):
        super(CLIPLoss, self).__init__()
        self.model, self.preprocess = clip.load("ViT-B/32", device="cuda")
        self.upsample = torch.nn.Upsample(scale_factor=7)
        self.avg_pool = torch.nn.AvgPool2d(kernel_size=stylegan_resolution // 32)
...
class Projector:
    def __init__(self, stylegan_size, text_input):
        self.stylegan_size              = stylegan_size
        self.text_inputs = torch.cat([clip.tokenize(text_input)]).cuda()
        self.num_steps                  = 1000
    ...
    def set_network(self, Gs, dtype='float16'):
        self.clip = CLIPLoss(self.stylegan_size)
        if Gs is None:
            self._Gs = None
            return
...
def project(network_pkl: str, target_fname: str, outdir: str, save_video: bool, seed: int, stylegan_size: int):
    # Load networks.
...
def main():
    ...
    parser.add_argument('--stylegan_size',help='Stylegan Resolution', dest='stylegan_size', default=1024, type=int)
    parser.add_argument('--target',      help='Target Text to Proyect', dest='target_fname', required=True)
